...
class PlaylistCreator:
    ...
    async def _scrape_remote(self) -> list[FoundAceStream]:
        found_html_streams = await self._html_scraper.scrape_sites(
            sites=self._conf.scraper.html
        )
        found_iptv_streams = await self._iptv_scraper.scrape_iptv_playlists(
            sites=self._conf.scraper.iptv_m3u8
        )
        found_api_streams = await self._api_scraper.scrape_api_endpoints(
            sites=self._conf.scraper.api
        )
        logger.info(
        ...
        found_streams_unique = list(
            create_unique_stream_list(found_streams).values()
        )  # Unique list of content_ids
        found_streams_infohash_only = [
            stream
            for stream in found_streams
            if stream.infohash and not stream.content_id
        ]
    ...
    def _scrape_existing(self) -> list[FoundAceStream]:
        ...
        ace_playlist_content_id = (
            self._output_directory / f"{self.playlist_name}-ace.m3u"
        )
        site = ScrapeSiteIPTV(
        ...
        if ace_playlist_content_id.exists():
            with ace_playlist_content_id.open("r", encoding="utf-8") as m3u_file:
                ...
                content_id_results = self._iptv_scraper.parse_m3u_content(
                    content=content, site=site
                )

        ace_playlist_infohash = (
            self._output_directory / f"{self.playlist_name}-local-infohash.m3u"
        )
        site = ScrapeSiteIPTV(
        ...
        if ace_playlist_infohash.exists():
            with ace_playlist_infohash.open("r", encoding="utf-8") as m3u_file:
                content = m3u_file.read()
                infohash_results = self._iptv_scraper.parse_m3u_content(
                    content=content, site=site
                )
    ...
    def _create_playlist(self, streams: list[FoundAceStream]) -> None:
        ...
        existing_content_id: list[str] = [
            stream.content_id for stream in existing_streams if stream.content_id != ""
        ]
        existing_infohash: list[str] = [
            stream.infohash for stream in existing_streams if stream.infohash != ""
        ]
        ...
        if infohash_scheme not in M3U_URI_SCHEMES:
            logger.error(
                "Infohash URI scheme not found in M3U_URI_SCHEMES, the developer needs to fix this."
            )
            return

        for uri_scheme, prefix in M3U_URI_SCHEMES.items():
            playlist_path = (
                self._output_directory / f"{self.playlist_name}-{uri_scheme}.m3u"
            )
            with playlist_path.open("w", encoding="utf-8") as m3u_file:
                ...
                for stream in streams:
                    top_line = create_extinf_line(
                        stream, tvg_url_base=self._conf.scraper.tvg_logo_external_url
                    )
